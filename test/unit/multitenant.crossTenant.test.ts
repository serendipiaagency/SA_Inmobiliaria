...
const RESOURCE_ROWS: Record<string, (f: TenantFixture, tag: string) => Record<string, any>> = {
  ...
  'developer-property-rooms': (f, tag) => ({ developerPropertyId: f.projectId, type: 'Dormitorio', name: `${tag} room` }),
  'agent-property-rooms': (f, tag) => ({ propertyId: f.propertyId, type: 'Dormitorio', name: `${tag} room` }),
