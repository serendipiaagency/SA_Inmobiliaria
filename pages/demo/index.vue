<template>
  <div>
    ...
    <section v-reveal class="border-y border-line bg-white">
      <div class="mx-auto grid max-w-screen-2xl gap-10 px-6 py-16 lg:grid-cols-2 lg:items-center lg:px-10">
        <div>
          ...
          <NuxtLink to="/demo/properties" class="btn-primary mt-8">{{ t('home.map.cta') }}</NuxtLink>
        </div>
        <NuxtLink to="/demo/properties" class="group relative block h-72 overflow-hidden rounded-2xl border border-line md:h-96">
    ...
    <!-- 3 · Destacados -->
    <SectionRow :eyebrow="t('home.featured.eyebrow')" :title="t('home.featured.title')" :items="destacados" to="/demo/properties" :cta="t('home.featured.cta')" />

    <!-- 4 · Últimas incorporaciones -->
    <div class="border-t border-line">
      <SectionRow :eyebrow="t('home.latest.eyebrow')" :title="t('home.latest.title')" :items="latest" to="/demo/properties?sort=newest" />
    </div>
    ...
    <section v-reveal class="bg-ink py-16 text-white">
      <div class="mx-auto max-w-screen-2xl px-6 lg:px-10">
        <div class="mb-8 flex items-end justify-between">
          ...
          <NuxtLink to="/demo/properties?sort=price_desc" class="hidden shrink-0 border border-white/40 px-5 py-2.5 text-[11px] font-semibold uppercase tracking-widest2 transition hover:bg-white hover:text-ink md:inline-flex">{{ t('home.premium.cta') }}</NuxtLink>
        </div>
        <div class="grid gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">
          <NuxtLink v-for="p in premium" :key="p.id" :to="`/demo/property-details/${p.slug || p.id}`" class="group block">
    ...
    <section v-reveal class="mx-auto max-w-screen-2xl px-6 py-16 lg:px-10">
      ...
      <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <NuxtLink v-for="c in (data?.communities || []).slice(0, 6)" :key="c.id" :to="`/demo/community/${c.id}`" class="group relative block aspect-[3/2] overflow-hidden rounded-2xl bg-stone-100">
    ...
    <section v-if="propertyTypes.length" v-reveal class="border-y border-line bg-white py-16">
      <div class="mx-auto max-w-screen-2xl px-6 lg:px-10">
        ...
        <div class="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          <NuxtLink v-for="pt in propertyTypes" :key="pt" :to="{ path: '/demo/properties', query: { type: pt } }" class="flex items-center justify-between rounded-xl border border-line px-5 py-4 text-sm transition hover:border-ink hover:bg-paper">
    ...
    <section v-if="data?.blogs?.length" v-reveal class="mx-auto max-w-screen-2xl px-6 py-16 lg:px-10">
      <div class="mb-8 flex items-end justify-between">
        ...
        <NuxtLink to="/demo/blog" class="btn-quiet hidden md:inline-flex">{{ t('home.blog.cta') }}</NuxtLink>
      </div>
      <div class="grid gap-x-6 gap-y-10 md:grid-cols-3">
        <NuxtLink v-for="b in data.blogs" :key="b.id" :to="`/demo/blog/${b.slug}`" class="group block">
