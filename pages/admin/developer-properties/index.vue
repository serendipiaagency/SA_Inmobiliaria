<template>
  <div>
    <div class="mb-4 flex flex-wrap items-end justify-between gap-4">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">Propiedades (web)</h1>
        <p class="mt-1 text-sm text-stone-500">{{ data?.total ?? 0 }} propiedad{{ data?.total === 1 ? '' : 'es' }}</p>
      </div>
      <NuxtLink to="/admin/developer-properties/new" class="btn-primary">+ Nueva propiedad</NuxtLink>
    </div>

    <!-- Search + quick filters + view toggle -->
    <div class="mb-3 flex flex-wrap items-center gap-2">
      <div class="relative">
        <input v-model="q" class="input !w-64 !pl-8" placeholder="Nombre, referencia, dirección, zona…" @keyup.enter="applyAndReset" />
    ...
    <div v-else-if="view === 'grid'" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <AdminDeveloperPropertyCard v-for="p in data.rows" :key="p.id" :property="p" @publish="togglePublish" @duplicate="duplicate" @delete="remove" />
    </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr v-for="p in data.rows" :key="p.id" class="border-t border-line hover:bg-stone-50">
            <td class="px-4 py-3">
              <NuxtLink :to="`/admin/developer-properties/${p.id}`" class="flex items-center gap-2.5">
                <img v-if="p.coverImage" :src="mediaUrl(p.coverImage)" class="h-9 w-9 shrink-0 rounded object-cover" />
            ...
            <td class="px-4 py-3 text-stone-500">{{ [p.community, p.city, p.country].filter(Boolean).join(' · ') || '—' }}</td>
            ...
            <td class="px-4 py-3">
              <span class="rounded-full bg-stone-100 px-2 py-0.5 text-[11px] font-semibold text-stone-600">{{ STATUS_LABELS[p.status] || p.status }}</span>
              <span class="ml-1 rounded-full px-2 py-0.5 text-[11px] font-semibold" :class="p.publishedAt ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-500'">
                {{ p.publishedAt ? 'Publicada' : 'Sin publicar' }}
              </span>
            </td>
            <td class="px-4 py-3 text-stone-450">{{ p.updatedAt ? new Date(p.updatedAt).toLocaleDateString('es-ES') : '—' }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
              <NuxtLink :to="`/admin/developer-properties/${p.id}`" class="mr-2 font-medium text-stone-600 hover:underline">Editar</NuxtLink>
              <a :href="`/propiedades/${p.slug || p.id}`" target="_blank" rel="noopener" class="mr-2 font-medium text-stone-600 hover:underline">Preview</a>
              <button type="button" class="mr-2 font-medium text-stone-600 hover:underline" @click="togglePublish(p.id)">{{ p.publishedAt ? 'Despublicar' : 'Publicar' }}</button>
...
useHead({ title: 'Propiedades (web)' })

const { confirm } = useConfirm()
const toast = useToast()

const PROPERTY_TYPES = ['Apartment', 'Villa', 'Townhouse', 'Penthouse', 'Studio']
const STATUS_OPTIONS = ['new', 'under_construction', 'ready']
...
onMounted(() => {
  const saved = localStorage.getItem('sa-admin-developer-properties-view')
  if (saved === 'grid' || saved === 'list') view.value = saved
})
function setView(v: 'list' | 'grid') {
  view.value = v
  try {
    localStorage.setItem('sa-admin-developer-properties-view', v)
  } catch {}
}
...
const hasActiveFilters = computed(() => !!q.value || !!status.value || !!propertyType.value || advancedCount.value > 0)
...
const { data, pending, refresh } = await useFetch<any>('/api/admin/developer-properties', {
  query: computed(() => ({
    page: page.value,
    q: q.value,
    status: status.value,
...
watch([status, propertyType, sort, priceMin, priceMax, country, city, district, postalCode, bedroomsMin, bathroomsMin, areaMin, areaMax], () => (page.value = 1))
...
async function togglePublish(id: number) {
  ...
  const publishing = !row?.publishedAt
  try {
    await $fetch<{ ok: true }>(`/api/admin/developer-properties/${id}`, { method: 'PUT', body: { publishedAt: publishing ? new Date().toISOString() : null } })
    toast.success(publishing ? 'Propiedad publicada' : 'Propiedad despublicada')
    await refresh()
  } catch {
...
async function duplicate(id: number) {
  try {
    await $fetch<{ ok: true; id: number }>(`/api/admin/developer-properties/${id}/duplicate`, { method: 'POST' })
...
async function remove(id: number) {
  ...
  if (!ok) return
  try {
    await $fetch<{ ok: true }>(`/api/admin/developer-properties/${id}`, { method: 'DELETE' })
