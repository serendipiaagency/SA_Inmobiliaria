  ...
  const sections: HelpSection[] = [
    ...
    },
    {
      ...
      steps: [
        ...
        'Usa el mismo Property Builder por secciones numeradas que "Propiedades (web)" — Información básica, Ubicación, Precio, Características, Descripción, Multimedia, Galería y Comercial — adaptado a los campos de este catálogo (sin planos, tipos de unidad ni plan de pagos, al no aplicar a una vivienda de reventa individual). El estado de cada sección (completa/pendiente) y el porcentaje de progreso funcionan igual que en Propiedades (web).',
        'La sección "Ubicación" incluye ahora los mismos campos de dirección granular y el mapa interactivo (buscar dirección, arrastrar el marcador) que Propiedades (web). Si la propiedad ya tenía una dirección en el campo de texto libre anterior, se conserva como "Referencia de ubicación (heredado)" — no se pierde, y puedes rellenar los campos nuevos cuando quieras.',
