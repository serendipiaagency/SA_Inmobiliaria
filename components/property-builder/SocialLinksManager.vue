    ...
    <template v-else>
      ...
      <div class="space-y-2">
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            ...
            <input v-model="row.url" class="pe-input mt-1 !py-1 !text-[12px]" placeholder="https://…" @change="saveUrl(row)" >
          </div>
      ...
      <div v-if="picking" class="mt-3 rounded-lg border border-line bg-stone-50 p-3">
        ...
        <div v-if="newPlatform" class="mt-3 flex items-center gap-2">
          ...
          <input v-model="newUrl" type="url" class="pe-input flex-1" placeholder="https://…" @keyup.enter="confirmAdd" >
          <button type="button" class="pe-btn-dark !px-3 !py-1.5" :disabled="!newUrl" @click="confirmAdd">Añadir</button>
        </div>
        <button type="button" class="mt-2 text-[12px] font-medium text-stone-400 hover:text-ink" @click="cancelPick">Cancelar</button>
      </div>
      <button v-else type="button" class="pe-btn-quiet mt-3" @click="picking = true">+ Añadir red social</button>
