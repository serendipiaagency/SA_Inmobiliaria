<template>
  <section v-if="items.length" v-reveal class="mx-auto max-w-screen-2xl px-6 py-16 lg:px-10">
    <div class="mb-8 flex items-end justify-between">
      ...
      <NuxtLink v-if="content.cta" :to="content.ctaTo || '/demo/blog'" class="btn-quiet hidden md:inline-flex">{{ content.cta }}</NuxtLink>
    </div>
    <div class="grid gap-x-6 gap-y-10 md:grid-cols-3">
      <NuxtLink v-for="b in items" :key="b.id" :to="`/demo/blog/${b.slug}`" class="group block">
