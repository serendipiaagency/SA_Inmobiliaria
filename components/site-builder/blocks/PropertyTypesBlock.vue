  ...
  <section v-if="types.length" v-reveal class="border-y border-line bg-white py-16">
    <div class="mx-auto max-w-screen-2xl px-6 lg:px-10">
      ...
      <div class="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        <NuxtLink v-for="pt in types" :key="pt" :to="{ path: '/propiedades', query: { type: pt } }" class="flex items-center justify-between rounded-xl border border-line px-5 py-4 text-sm transition hover:border-ink hover:bg-paper">
