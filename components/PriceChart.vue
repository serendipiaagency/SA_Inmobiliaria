  ...
  <div class="rounded-2xl border border-line bg-white p-6 sm:p-8">
    ...
    <template v-else-if="points.length >= 2">
      <div class="mb-5 flex items-baseline justify-between">
        <div>
          <p class="text-[11px] font-semibold uppercase tracking-widest text-stone-450">Precio actual</p>
        ...
        <span v-if="changePct" class="rounded-full px-2.5 py-1 text-[11px] font-semibold" :class="changePct < 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'">
          {{ changePct > 0 ? '+' : '' }}{{ changePct }}% desde el registro anterior
        </span>
      </div>
      <AdminAreaChart :values="points.map((p) => p.price)" :labels="points.map((p) => dateLabel(p.recordedAt))" :h="200" :format="(v) => formatPrice(v)" />
    </template>
    <p v-else class="text-sm text-stone-500">Aún no hay suficientes registros de precio para mostrar una tendencia. El histórico se irá completando con cada actualización real del precio.</p>
  </div>
</template>
