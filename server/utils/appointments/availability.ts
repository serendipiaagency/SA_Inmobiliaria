 ...
 * rules, minus any time-off for that date, minus slots that would overlap an
 * existing non-cancelled visit, minus slots already in the past. This is the
 * one place both the public slot picker and the booking endpoint's own
 * server-side re-check go through, so a client can never book a slot the UI
 * never actually offered.
 */
export async function computeAvailableSlots(db: any, orgId: number, agentId: number, dateStr: string, slotDurationMinutes: number): Promise<Slot[]> {
  ...
  for (const rule of rules) {
    ...
    for (let t = windowStart; t + slotDurationMinutes <= windowEnd; t += slotDurationMinutes) {
      ...
      const blocked = busy.some((b: any) => overlaps(start, end, b.scheduledAt, b.endsAt || b.scheduledAt))
...
export async function isSlotAvailable(db: any, orgId: number, agentId: number, startDateTime: string, slotDurationMinutes: number): Promise<boolean> {
  const dateStr = startDateTime.slice(0, 10)
  const slots = await computeAvailableSlots(db, orgId, agentId, dateStr, slotDurationMinutes)
